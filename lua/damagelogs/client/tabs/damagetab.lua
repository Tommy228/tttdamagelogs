...
function PANEL:SetPlayer(nick)
	...
	self.Close.PaintOver = function(s, w, h)
		surface.SetFont("DermaDefault")

		local x, y = surface.GetTextSize("X")

		surface.SetTextPos(w * 0.5 - x * 0.5 + 1, h * 0.5 - y * 0.5)
		surface.DrawText("X")
	end
...
function Damagelog:DrawDamageTab(x, y)
	...
	self.Filters.DoClick = function(Filters, forcedX, forcedY)
		...
		local x2, y2 = gui.MouseX(), gui.MouseY()
		...
		for k in pairs(Damagelog.filters) do
			...
			local option = filters:AddOption(TTTLogTranslate(GetDMGLogLang, k), function()
				...
				timer.Simple(0, function()
					Filters:DoClick(forcedX or x2, forcedY or y2)
				end)
			end)
		...
		filters:Open(forcedX or x2, forcedY or y2, isnumber(forcedX))
	end
	...
	self.PlayerSelect.UpdatePlayers = function(s)
		for _, v in pairs(s.Panels) do
			v:Remove()
		end

		table.Empty(s.Panels)
		...
		if #Damagelog.Highlighted > 0 then
			...
			local x2 = surface.GetTextSize(Damagelog.PS_Label.Text)
			x2 = x2 + 10

			for _, v in ipairs(Damagelog.Highlighted) do
				local ply = vgui.Create("DL_FiltersPlayer", s)

				table.insert(s.Panels, ply)
	...
	self.PlayersCombo.Update = function(s)
		s:Clear()

		for k, v in pairs(s.Players) do
			s:AddChoice(v.nick, k)
		end

		if table.Count(s.Players) > 0 then
			s:ChooseOptionID(1)
			s:SetEnabled(true)
		else
			s:SetEnabled(false)
	...
	self.PlayersCombo.OnSelect = function(s, index, value, data)
		s.CurrentlySelected = value
	end
	...
	self.Highlight.DoClick = function(s)
		local _, selected = Damagelog.PlayersCombo:GetSelected()
	...
	for _, v in pairs(forms) do
		...
		v.Toggle = function(s2)
			if s2:GetExpanded() then
				return
			else
	...
	self.Round.OpenMenu = function(s, pControlOpener)
		if pControlOpener and pControlOpener == s.TextEntry then return end
		...
		for _, v in pairs(sorted, "data") do
			s.Menu:AddOption(v.data, function()
				s:ChooseOption(v.data, v.id)
			end)
		end

		local x2, y2 = s:LocalToScreen(0, s:GetTall())

		s.Menu:SetMinimumWidth(s:GetWide())
		s.Menu:Open(x2, y2, false, s)
	end
end
