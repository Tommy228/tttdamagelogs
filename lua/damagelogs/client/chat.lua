...
function Damagelog:StartChat(report, admins, victim, attacker, players, history, adminReport)
	...
	Actions.DoClick = function(slf)
		...
		menu:AddOption(TTTLogTranslate(GetDMGLogLang, "AddPlayer"), function()
			...
			plist.AddPlayer = function(pnl, pl2)
				local pl = pl2
				...
				ply.OnMousePressed = function(_, mc)
	...
	Chat.AddPlayer = function(_, ply, category)
		List:AddPlayer(ply, category)
	end

	Chat.RemoveAdmin = function(_, admin)
	...
	RichText.AddText = function(slf, nick, color, text)
		slf.m_FontName = "DL_ChatFont"

		slf:SetFontInternal("DL_ChatFont")
		slf:InsertColorChange(color.r, color.g, color.b, color.a or 255)
		slf:AppendText(nick.. ": ")
		slf:InsertColorChange(255, 255, 255, 255)
		slf:AppendText(text.."\n")
	end
	...
	Chat.Stop = function(slf)
		TextEntry:SetDisabled(true)
		Actions:SetDisabled(true)
		slf:SetDeleteOnClose(true)
	end

	Chat.AddMessage = function(_, msg)
	...
	TextEntry.OnEnter = function(slf)
		SendMessage(slf:GetValue())
	end
	...
	timer.Simple(0.1, function()
		for _, v in ipairs(history) do
			RichText:AddText(v.nick, v.color, v.msg) -- TODO The function is deprecated
		end
...
net.Receive("DL_BroadcastMessage", function()
	...
	for _, v in pairs(Damagelog.CurrentChats) do
		if v.RID == id then
			...
			v.RichText:AddText(ply:Nick(), color, message) -- TODO The function is deprecated
...
net.Receive("DL_ViewChatCL", function()
	...
	local RichText = vgui.Create("RichText", Frame) -- TODO The function is deprecated
...
hook.Add("HUDPaint", "Damagelog_Chat", function()
	...
	if IsValid(client) then
		...
		if not drawing and #Damagelog.CurrentChats > 0 then
			...
			Damagelog.ChatButton.PaintOver = function(self, w2, h2)
				surface.SetFont("DermaDefault")

				local text = show_chats and "▼" or "▲"
				local wt, ht = surface.GetTextSize(text)

				surface.SetTextPos(w2 / 2 - wt / 2 + 2, h2 / 2 - ht / 2)
		...
		if #Damagelog.CurrentChats > 0 then
			...
			local _, ht = surface.GetTextSize(text)
			...
			if show_chats then
				...
				for _, v in pairs(Damagelog.CurrentChats) do
					local w2 = surface.GetTextSize(v.VictimName .. " " .. andStr .. " " .. v.AttackerName)
					if w2 > max_w then
						max_w = w2
					end
				end
				...
				for k, v in ipairs(Damagelog.CurrentChats) do
					...
					local w_and = surface.GetTextSize(andStr)
