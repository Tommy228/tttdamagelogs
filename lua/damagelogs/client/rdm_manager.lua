...
local function BuildReportFrame(report)
	if IsValid(ReportFrame) and report then
		...
		ReportFrame.AddReport = function(slf, rep)
			local current = not rep.previous
			...
			local txt = ""
			local nick = rep.adminReport and TTTLogTranslate(GetDMGLogLang, "AnAdministrator") or rep.victim_nick

			if current then
				txt = string.format(TTTLogTranslate(GetDMGLogLang, "HasReportedCurrentRound"), nick, rep.round or "?")
			else
			...
			MessageEntry:SetText(rep.message or "")
				...
				if size < 10 then
					...
					timer.Create("TimerRespond", 5, 1, function()
						if IsValid(Info) then
							Info:SetText(rep.victim_nick .. " " .. TTTLogTranslate(GetDMGLogLang, "ReportedYou") .. (current and (" " .. TTTLogTranslate(GetDMGLogLang, "AfterRound") .. " " .. (rep.round or "?")) or " " .. TTTLogTranslate(GetDMGLogLang, "PreviousMap")))
							Info:SetInfoColor("blue")
						end
					end)
				else
			...
			local title = rep.adminReport and string.format(TTTLogTranslate(GetDMGLogLang, "AdminReportID"), rep.index) or rep.victim_nick
		...
		for _, rep in ipairs(Damagelog.ReportsQueue) do
			if not rep.finished then
				ReportFrame:AddReport(rep)
			end
		end
...
function Damagelog:ReportWindow(found, deathLogs, previousReports, currentReports, dnas)
	...
	UserList.Paint = function(_, w2, h2)
	...
	local function UpdateDNAMessage(ply)
		...
		local h2 = select(2, surface.GetTextSize(msg))

		DNAMessage:SetPos(35, DNAMessage:GetParent():GetTall() / 2 - h2 / 2)
	end
	...
	UserList.AddPlayer = function(pnl, pl, is_killer, killer_valid)
		...
		ply.OnSelected = function(p)
			for _, v in pairs(pnl:GetItems()) do
				if v.Selected and v ~= p then
					v.Selected = false
				end
			end
	...
	DNAPanel.Paint = function(_, w2, h2)
		surface.SetDrawColor(color_black)
		surface.DrawRect(0, 0, w2, h2)

		surface.SetDrawColor(Color(225, 225, 225))
		surface.DrawRect(1, 1, w2 - 2, h2 - 2)
	end
	...
	Type.OnSelect = function(_, data, text)
	...
	RoundsList.Paint = function(_, w2, h2)
		surface.SetDrawColor(Color(52, 73, 94))
		surface.DrawRect(0, 0, w2, h2)
	end
	...
	VictimInfos.Paint = function(panel, w2, h2)
		local bar_height = 27

		surface.SetDrawColor(30, 200, 30)
		surface.DrawRect(0, 0, w2 / 2, bar_height)
		draw.SimpleText(TTTLogTranslate(GetDMGLogLang, "YourReport"), "DL_RDM_Manager", w2 / 4, bar_height / 2, Color(0, 0, 0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

		surface.SetDrawColor(220, 30, 30)
		surface.DrawRect(w2 / 2 + 1, 0, w2 / 2, bar_height)
		draw.SimpleText(TTTLogTranslate(GetDMGLogLang, "PlayerResponse"), "DL_RDM_Manager", w2 / 2 + 1 + w2 / 4, bar_height / 2, Color(0, 0, 0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

		surface.SetDrawColor(0, 0, 0)
		surface.DrawOutlinedRect(0, 0, w2, h2)
		surface.DrawLine(w2 / 2, 0, w2 / 2, h2)
		surface.DrawLine(0, 27, w2, bar_height)

		if panel.DisableR then
			surface.SetDrawColor(color_trablack)
			surface.DrawRect(w2 / 2 + 1, 0, w2 / 2, h2)
		end
	end
	...
	KillerMessage.PaintOver = function(slf, w2, h2)
		if slf.DisableR then
			...
			surface.SetTextPos(w2 / 2 - (wt - 14) / 2, h2 / 3 - ht / 2 + 10)
			surface.DrawText(text)

			surface.SetMaterial(Material("icon16/exclamation.png"))
			surface.SetDrawColor(color_white)
			surface.DrawTexturedRect(w2 / 2 - wt / 2 - 14, h2 / 3 - ht / 2 + 10, 16, 16)
		end
	end
	...
	conclusionPanel.Paint = function(_, w2, h2)
		surface.SetDrawColor(color_black)
		surface.DrawRect(0, 0, w2, h2)
	...
	if #previousReports > 0 then
		...
		for _, report in ipairs(previousReports) do
			...
			button.OnSelected = function(btn)
				self.ReportsInfo.ViewingPrevious = true
				self.ReportsInfo.CurrentIndex = btn.report.index

				net.Start("DL_AskOwnReportInfo")
				net.WriteUInt(1, 1)
				net.WriteUInt(btn.report.index, 16)
				net.SendToServer()
