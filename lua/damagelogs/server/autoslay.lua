...
if not sql.TableExists("damagelog_autoslay") then
	sql.Query([[
		CREATE TABLE damagelog_autoslay (
		...
		if v:IsActive() then
			...
			if data then
				...
				if IsValid(v.server_ragdoll) then
					local ply
					if TTT2 then
						ply = player.GetBySteamID64(v.server_ragdoll.sid64)
					else
						ply = player.GetBySteamID(v.server_ragdoll.sid)
					end
